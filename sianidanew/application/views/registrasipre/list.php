...
<script>
    ...
    function recordDelete(i) {
        if (confirm("Are you sure you want to delete this?")) {
            $("#delete-button-" + i).attr("href", "<?= site_url('regprepaid/hapus/') ?>" + i);
        } else {
            return false;
        }
                        ...
                        <table class="table">
                            ...
                            <?php foreach ($logbook[0]->result() as $row) { ?>
                                <tr>
                                    ...
                                    <td><a href="<?= site_url('regprepaid/detail/' . $row->id) ?>" class="btn btn-primary btn-xs">view</a>
                                        <?php if ($this->ion_auth->in_group(array(1))) { ?>
                                        <a href="#" class="btn btn-danger btn-xs" id="delete-button-<?= $row->id ?>" onclick="recordDelete(<?= $row->id ?>)">delete</a></td>
                                    <?php } ?>
                                </tr>
                            <?php } ?>
                            </tbody>
